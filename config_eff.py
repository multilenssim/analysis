	...
	for cfg in sorted(glob.glob('/farmshare/user_data/jdalmass/S*/')):
		...
		try:
			with h5py.File(cfg+sl+'e-_sim.h5','r') as f:
				...
				ax1.errorbar(px_per_lens,np.mean(e_index/np.mean(tot_index)),np.std(e_index/np.mean(tot_index))/np.sqrt(len(e_index)),fmt='o')
...
ax1.fill_between(line11[:,0],line11[:,1],line21[:,1],facecolor='green',alpha=0.5, label='EPD ratio = 1.0')
ax1.fill_between(line12[:,0],line12[:,1],line22[:,1],facecolor='red', alpha=0.5, label='EPD ratio = 0.8')
